...
The Original Code is: msEditIf.pas
...
uses
  ...
  Dialogs, StdCtrls, msGlobals,
  msprogram;

type
  TFormEditIf = class(TForm)
    cbVars: TComboBox;
    Label1: TLabel;
    edValue: TEdit;
    lbContent: TLabel;
...
function DlgEditIf(M: TIFBlock): Boolean;
...
function DlgEditIf(M: TIFBlock): Boolean;
begin
  with TFormEditIf.Create(nil) do
  try
    Result := False;
...
procedure TFormEditIf.FormCreate(Sender: TObject);
begin
  Varhandler.GetVarList(cbVars.Items);
end;

procedure TFormEditIf.cbVarsChange(Sender: TObject);
begin
