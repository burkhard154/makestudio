...
resourcestring
  ...
  struPluginHint = 'Delphi Plugin for D5,D7,D9,D10,XE,XE2,XE3,XE4,XE5';
...
resourcestring
  ...
  stdBPLDirNotInPath     = 'Selected BPL directory not in Search path.'#10#13+
                           'Shall it be added?';
  ...
  stdBPLPathNotExists = 'BPL Path for Delphi Version "%s" does not exist.'#10#13+
...
type
  TDMakAction = (dmaNone,
                 dmaRegDPK, dmaUnRegDPK,
                 dmaRegCOMDll, dmaUnRegCOMDll,
                 dmaRegCOMExe, dmaUnRegCOMExe,
                 dmaSetSearchPath, dmaCompile,
                 dmaRunBat, dmaRegisterDllExpert,
                 dmaDeleteBPL, dmaDeleteDCP,
                 dmaRemoveSearchPath, dmaRemoveSearchPathIncludingSubdir,
                 dmaRemoveAllUserPackages, dmaResetSearchPath);

  TGetVarOperationType = ( otBPLDir, otDCPDir, otRootPath, otSearchPath, otBDSPROJECTSDIR);


  TDelphiVersion = (dver5, dver6, dver7, dver2005, dver2006, dver2007, dver2009, dver2010, dverXE, dverXE2, dverXE3, dverXE4, dverXE5, dverXE6, dverXE7);
...
var
  stActions: array [dmaNone..dmaResetSearchPath] of string =
    (stdmaNone, stdmaRegDPK, stdmaUnRegDPK, stdmaRegCOMDll, stdmaUnRegCOMDll,
     stdmaRegCOMExe, stdmaUnRegCOMExe, stdmaSetSearchPath, stdmaCompile,
     stdmaRunBat, stdmaRegisterDllExpert, stdmaDeleteBPL, stdmaDeleteDCP,
     stdmaRemoveSearchPath, stdmaRemoveSearchPathIncludingSubdir,
     stdmaRemoveAllUserPackages, stdmaResetSearchPath
     );

  stGetVarActions : array[otBPLDir..otBDSPROJECTSDIR] of String =
    (stotBPLDir, stotDCPDir, stotRootPath, stotSearchPath, stotBDSPROJECTSDIR);
