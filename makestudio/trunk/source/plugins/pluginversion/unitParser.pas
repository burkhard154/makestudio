...
begin
  fReader := TStreamReader.Create (AStream);
  fWhitespace := [' ', #9, #10];
...
begin
  if tokenType = ttNumber then
    ...
    raise EParser.CreateFmt( 'Line %d: '+errMsg, [fLineNo]);
end;
...
begin
  ...
  repeat
    ...
    begin
//      if not fReader.ReadLine(fLineBuf) then
//      begin
//        fCh := #0;
//        result := fCh;
//        exit
